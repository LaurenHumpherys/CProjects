 ...
 */
int readData(int footballData[MAXPLAYERS][DATACOLS]) {
    ...
    //inFile = fopen("C:\\Users\\C22Lauren.Humpherys\\CLionProjects\\CProjects\\Labs\\Lesson15\\fbData.txt", "r");
    //inFile = fopen("../Labs/Lab15/fbData.txt", "r"); // use this for working in CLion (comment when you submit)
    ...
    while (!feof(inFile) && numRead < MAXPLAYERS) {
        fscanf(inFile, "%s %s %d %d %d %d", firstName, lastName, &gamesPlayed[numRead],
                &(footballData[numRead][0]),&(footballData[numRead][1]),&(footballData[numRead][2]));
        strcat(firstName, " ");
